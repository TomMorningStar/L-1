...
function evaluatePasswordComplexity(password) {
    ...
    else if (hasLowercase || hasUppercase) complexity += 1;
    // проверка на цифры
    const hasNumbers = /\d/.test(password);
    if (hasNumbers) complexity += 1;
    // проверка на спецсимволы
    const hasSpecialCharacters = /[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/.test(password);
    if (hasSpecialCharacters) complexity += 2;
    // Evaluate overall password complexity
    if (length >= 8 && complexity >= 3) return "Сильный пароль";
    else if (length >= 6 && complexity >= 2) return "Пароль средний";
    else return "Слабый пароль. Рассмотрите возможность увеличения длины и использования сочетания строчных и прописных букв, цифр и специальных символов.";
}
...
//# sourceMappingURL=evaluatePassword.c36efa7c.js.map
